...
class Admin::UsersControllerTest < ActionController::TestCase
  ...
  def test_should_process_account_request_and_update_ci_notes
    login_as :admin
    put :process_account_request,
      :account_request_id => user_account_requests(:request1).id,
      :user_account_request => { :ci_notes => 'updated_notes' }
    assert_redirected_to :action => 'list_account_requests'
    user_account_requests(:request1).reload
    assert_match 'updated_notes', user_account_requests(:request1).ci_notes
  end
  ...
  def test_should_allow_admin_access_to_show_account_request
    login_as :admin
    get :show_account_request, :id => user_account_requests(:request1)
    assert_response :success
    assert_select "[class=errorExplanation]", false
  end
